...
username.toLowerCase() });
 ...
 if (!avatar) {
 avatar =
`https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(username)}&backgroundColor=00f2fe,4facfe&color=ffffff`;
 }
 // Save User
 let user;
 if (isMongoConnected) {
...
app.post('/api/groups', authenticateToken, async (req, res) => {
 ...
 const groupAvatar =
`https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(name.trim())}&backgroundColor=6a11cb,2575fc&color=ffffff`;
 let newGroup;
 if (isMongoConnected) {
 newGroup = new Group({
 name: name.trim(),
...
// Map of active connections: username -> Set of sockets (handles multiple tabs for the same user)
...
async function broadcastOnlineUsers() {
 ...
 // Construct detail list of online users by getting their avatars from currently connected sockets (or fallback)
...
err.message);
 ...
 console.log(`[WS] User ${username} connected. Total client tabs: ${userSockets.get(username).size}`);
 ...
 ws.send(JSON.stringify({
 type: 'system',
 message: `Welcome to the secure chat, @${username}! Connection authenticated.`
 }));
 ...
 avatar: `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(m.sender)}&backgroundColor=00f2fe,4facfe&color=ffffff` // Fallback/generate
 }));
 } else {
...
targetRoomId);
 ...
 console.log(`[WS] Closed a tab for ${username}. Remaining tabs: ${socketSet.size}`);
 }
 }
 });
});
...
server.listen(PORT, () => {
 ...
 console.log(`[SYSTEM] Ready for Local Sandbox connection! Mocks enabled for zero-config startup.`);
});